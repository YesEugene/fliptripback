/**
 * Script to generate 10 sample tours in the database
 * Run with: node scripts/generate-sample-tours.js
 */
...
// Main function to generate tours
  ...
  for (const tourData of sampleTours) {
    try {
      ...
      for (const day of tourData.daily_plan) {
        const { data: tourDay, error: dayError } = await db
          .from('tour_days')
          .insert({
            tour_id: tour.id,
            day_number: day.day,
        ...
        for (const block of day.blocks) {
          const { data: tourBlock, error: blockError } = await db
            .from('tour_blocks')
            .insert({
          ...
          for (let i = 0; i < block.items.length; i++) {
            ...
            if (existingLocation) {
              ...
              const { data: newLocation, error: locError } = await db
                .from('locations')
                .insert({
                  name: item.title,
                  city_id: cityId,
